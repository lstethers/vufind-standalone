...
/* VUFINDLOCALONLY - BEGIN added check that location exists first */
		if (array_key_exists('location', $copy)) {
/* VUFINDLOCALONLY - END added check that location exists first */
                	$show = !in_array($copy['location'], $this->hideHoldings);
	                if ($show) {
	                    $groupKey = $this->getHoldingsGroupKey($copy);
	                    $holdings[$groupKey][] = $copy;
	                }
/* VUFINDLOCALONLY - BEGIN ending if statement above */
		}
/* VUFINDLOCALONLY - END ending if statement above */
            }
        }
        return $holdings;
    }
